...
if(isset($_GET['delete_id'])) {
    ...
    if(mysqli_query($conn, $sql)) {
        header("Location: suppliers.php?message=تم حذف المورد بنجاح");
        exit();
    } else {
        header("Location: suppliers.php?error=تعذر حذف المورد لارتباطه بمنتجات");
        exit();
    }
}

// جلب جميع الموردين مع عدد المنتجات المرتبطة بكل مورد
...
<head>
    ...
    <title>إدارة الموردين - نظام إدارة المخزون المبسط</title>
...
<body>
    <div class="container">
        ...
        <div class="page-header">
            <h1 class="page-title"><i class="fas fa-truck"></i> إدارة الموردين</h1>
            <button onclick="showAddForm()" class="btn btn-primary"><i class="fas fa-plus"></i> إضافة مورد جديد</button>
        </div>
        ...
        <div class="form-container" id="addForm" style="display: none;">
            <h2 style="margin-bottom: 1.5rem;">إضافة مورد جديد</h2>
        ...
        <div class="table-container">
            <table>
                ...
                <tbody>
                    <?php if(count($suppliers) > 0): ?>
                        <?php foreach($suppliers as $supplier): ?>
                            <tr>
                                ...
                                <td><span class="badge badge-success"><?php echo $supplier['products_count']; ?></span></td>
                                <td>
                                    <div class="action-buttons">
                                        <a href="edit_supplier.php?id=<?php echo $supplier['id']; ?>" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i></a>
                                        <a href="suppliers.php?delete_id=<?php echo $supplier['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('هل أنت متأكد من حذف هذا المورد؟')"><i class="fas fa-trash"></i></a>
                                    </div>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="6" style="text-align: center;">لا يوجد موردون مسجلون</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
